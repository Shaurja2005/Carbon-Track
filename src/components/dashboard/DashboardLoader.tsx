...
import { loadHistory, loadInput, loadGoal, type FootprintInput, type HistoryEntry, type Goal } from '@/lib';
...
type State =
  | { phase: 'loading' }
  | { phase: 'empty' }
  | { phase: 'ready'; input: FootprintInput; history: HistoryEntry[]; goal: Goal | null };

/**
 * Client shell for the dashboard. Reads the saved answers, history, and goal from
...
export function DashboardLoader(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setState({ phase: 'ready', input, history: loadHistory(), goal: loadGoal() });
  }, []);
  ...
  return <DashboardView input={state.input} history={state.history} goal={state.goal} />;
}
