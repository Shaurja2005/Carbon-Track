...
// Extracted Component: RecentHistoryList
function RecentHistoryList({ history }: { history: ChallengeEntry[] }): JSX.Element | null {
        ...
        {history.map((entry) => {
          ...
          return (
            <div
              ...
              title={entry.weekKey}
            >
              <Icon name={entry.status === 'done' ? 'check' : 'x'} size={12} />
...
// Extracted Component: CurrentChallengeCard
...
function CurrentChallengeCard({ challenge, weekKey, status, onAction }: CurrentChallengeCardProps): JSX.Element {
  ...
  return (
    ...
    >
      <span className="absolute right-5 top-5 text-xs font-mono text-ink-subtle">{weekKey}</span>
        ...
        <div className="flex flex-col gap-1.5 flex-1">
          ...
          {status === 'todo' && (
            <div className="mt-3 flex flex-wrap gap-2">
              <Button id="challenge-done-btn" size="md" variant="primary" onClick={() => onAction('done')}>
                <Icon name="check" size={16} />
                {challenge.actionLabel}
              </Button>
              <Button id="challenge-skip-btn" size="md" variant="ghost" onClick={() => onAction('skipped')}>
                Skip this week
              </Button>
            </div>
          )}
          ...
          {status === 'skipped' && <p className="mt-2 text-sm text-ink-subtle">Skipped this week.</p>}
        </div>
      </div>
    </div>
  );
...
export function WeeklyChallenges({ input, result }: WeeklyChallengesProps): JSX.Element {
  ...
  if (!loaded) {
    return <div className="h-48 animate-pulse rounded-3xl border border-border bg-primary/5" aria-hidden="true" />;
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <RecentHistoryList history={recentHistory} />
    </div>
  );
}
